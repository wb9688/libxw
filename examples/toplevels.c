...
static void notify_activated_event(XwToplevel *toplevel, GParamSpec *pspec, GtkWidget *button) {
    gtk_widget_set_sensitive(button, !xw_toplevel_get_activated(toplevel));
}
...
static void activate_clicked_event(GtkButton *button, XwToplevel *toplevel) {
    xw_toplevel_activate(toplevel);
}
...
static void new_toplevel_event(XwToplevels *xw_toplevels, XwToplevel *toplevel) {
    ...
    GtkWidget *activate_button = gtk_button_new_with_label("Activate");
    gtk_widget_set_sensitive(activate_button, !xw_toplevel_get_activated(toplevel));
    g_signal_connect(activate_button, "clicked", G_CALLBACK(activate_clicked_event), toplevel);
    gtk_container_add(GTK_CONTAINER(hbox), activate_button);
    ...
    g_signal_connect(toplevel, "notify::activated", G_CALLBACK(notify_activated_event), activate_button);
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    if (!xw_toplevels) {
        ...
        for (GList *toplevel = toplevels; toplevel; toplevel = toplevel->next) {
            ...
            GtkWidget *activate_button = gtk_button_new_with_label("Activate");
            gtk_widget_set_sensitive(activate_button, !xw_toplevel_get_activated(data));
            g_signal_connect(activate_button, "clicked", G_CALLBACK(activate_clicked_event), data);
            gtk_container_add(GTK_CONTAINER(hbox), activate_button);
            ...
            g_signal_connect(data, "notify::activated", G_CALLBACK(notify_activated_event), activate_button);
